...
def extract_key_errors(log_text: str, max_errors=5) -> str:
    # Use regex to extract all lines containing the word "error:" (case-insensitive)
    error_lines = re.findall(r'^.*error:.*$', log_text, re.IGNORECASE | re.MULTILINE)
    # Optionally limit the number of errors returned
